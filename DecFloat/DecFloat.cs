...
public class DecFloat {
    private const long FOUR_GIG = uint.MaxValue + 1L;
    ...
    public static DecFloat Zero { get; } = new(false, Array.Empty<uint>(), 0);
    public static DecFloat One { get; } = new(false, new uint[] { 1 }, 0);
    public static DecFloat Two { get; } = new(false, new uint[] { 2 }, 0);
    public static DecFloat Half { get; } = new(false, new uint[] { 5 }, 1);
    ...
    private static uint[] Sub(uint[] a, uint[] b) {
        ...
        if (c != 0) 
            throw new Exception("unexpected borrow");
        var rl = r.Length;
        while (rl > 0 && r[rl - 1] == 0) 
            rl--;
        if (rl != r.Length) 
            Array.Resize(ref r, rl);
        return r;
    }
    ...
    public DecFloat Div(DecFloat b, int maxDigitsOfPrecision) {
        ...
        if (rdp < 0) {
            result.Insert(0, new string('0', -rdp));
            rdp = 0;
        }
        if (rdp > result.Length) 
            result.Append(new string('0', rdp - result.Length));
    ...
    public DecFloat Log2(int precisionDigits) {
        if (this.Compare(Zero) <= 0)
            throw new ArgumentOutOfRangeException(nameof(DecFloat), "Cannot take log of 0 or negative number");
        var exp = Zero;
        var num = this.Clone();
    ...
    public DecFloat Sqrt(int precisionDigits) {
        if (this.Compare(Zero) < 0)
            throw new ArgumentOutOfRangeException(nameof(DecFloat), "Cannot take square root of negative number");
        ...
        for (;;) {
            ...
            if (cmp.neg) 
                a = ah;
            h = h.Mul(Half);
            if (!cmp.neg && cmp.Compare(limit) < 0) 
                break;
        }
    ...
    public static DecFloat Pi(int precisionDigits) {
        ...
        for (double i = 1; i < precisionDigits; i *= 1.9) {
            ...
            if (limit.Compare(pn.Sub(p).Abs()) > 0) 
                break;
            a = an;
            b = bn;
            p = pn;
    ...
    public static DecFloat Fact(uint n) {
        ...
        for (uint i = 2; i <= n; i++) {
            ...
            if (fnext > FOUR_GIG) {
                fact = fact.Mul(new DecFloat(false, new[] { fsub }, 0));
                fsub = i;
            } else
                fsub = (uint)fnext;
        }
        return fact.Mul(new DecFloat(false, new[] { fsub }, 0));
    }
